...
import { Sparkles, Trophy, Share2, Bookmark, ArrowRight, CheckCircle2 } from "lucide-react";
...
const AIVerdict = ({ itemA, itemB, weights }: AIVerdictProps) => {
  ...
  const recommendations = winner.category === 'tech' ? [
    ...
    "Book at least 45 days in advance for 'Hub Node' connectivity rates.",
    "Secure 'Safety Tier 1' accommodation in the central district.",
    "Sync local maps for offline neural navigation."
  ];
          ...
          <div className="flex-1 text-center lg:text-left space-y-6">
            <div>
              <h3 className="text-4xl font-black mb-4 tracking-tighter uppercase italic leading-tight">
                Neural Match: <br/> <span className="text-blue-400">{winner.name}</span>
              </h3>
              <p className="text-white/60 text-sm leading-relaxed font-medium">
                Based on your <span className="text-white font-bold italic">Neural Calibration</span>, 
                {winner.name} achieves superior trait-alignment. {winner.name} dominates in core priorities 
                while maintaining stability in secondary sectors.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-4">
              ...
              <Button className="rounded-full bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-black uppercase h-8 px-6">
