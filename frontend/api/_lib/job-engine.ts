...
async function runFactStep(
  ...
  try {
    ...
    const entityByNormName = new Map(entityRows.map(e => [e.normalizedName, e]));
    ...
    for (const [index, fact] of uniqueFacts.entries()) {
      ...
      const entityRow =
        entityByNormName.get(entityName) ||
        entityRows[0];
...
async function runScoreStep(
  ...
  try {
    ...
    for (const score of result.data) {
      const entityRow =
        entityByNormName.get(score.entity) ||
        entityRows[0];
      const dimensionRow =
...
function buildResultJson(
  ...
  const scoresByDimEntity = new Map<string, number>();
  for (const s of scores) {
    scoresByDimEntity.set(`${s.dimension}::${s.entity}`, s.score);
  }
  ...
  return {
    ...
    categories: dimensions.map((dim) => {
      const aScore = scoresByDimEntity.get(`${dim.name}::${entityA?.name}`) ?? 0;
      const bScore = scoresByDimEntity.get(`${dim.name}::${entityB?.name}`) ?? 0;
    ...
    dimensions: dimensions.map((dim) => {
      return {
        subject: dim.name,
        a: scoresByDimEntity.get(`${dim.name}::${entityA?.name}`) ?? 50,
        b: scoresByDimEntity.get(`${dim.name}::${entityB?.name}`) ?? 50,
        fullMark: 100,
      };
    }),
...
async function buildPartialResult(
  ...
  try {
    ...
    const scoresByDimEntityId = new Map<string, number>();
    for (const s of scores) {
      const dimName = dimsById.get(s.dimensionId);
      if (dimName) {
        scoresByDimEntityId.set(`${dimName}::${s.entityId}`, s.score);
      }
    }

    const categories = Array.from(factsByDim.entries()).map(([name, dimFacts]) => {
      const aScore = scoresByDimEntityId.get(`${name}::${entityA.id}`) ?? 50;
      const bScore = scoresByDimEntityId.get(`${name}::${entityB.id}`) ?? 50;
      return {
        name,
        winner: aScore > bScore ? "a" : bScore > aScore ? "b" : "tie",
