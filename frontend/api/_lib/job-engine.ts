  ...
  try {
    ...
    for (const fact of uniqueFacts) {
      ...
      const sourceRow =
        ...
        (citation ? sourceRows.find((source) => citation.includes(source.url)) : undefined) ||
        sourceRows[0];
...
async function runVerdictStep(
  ...
  try {
    ...
    const entityRows = await ctx.db
      ...
      .where(eq(comparisonEntities.comparisonId, ctx.comparisonId));
    const entityByNormalizedName = new Map(entityRows.map((e) => [e.normalizedName, e]));
    const winnerEntity = result.data.winner ? entityByNormalizedName.get(result.data.winner) : undefined;
...
function buildResultJson(
  ...
  for (const s of scores) {
    scoreLookup.set(`${s.dimension}::${s.entity}`, s.score);
  }
  ...
  return {
    ...
    categories: dimensions.map((dim) => {
      const aScore = scoreLookup.get(`${dim.name}::${entityA?.name}`) ?? 0;
      const bScore = scoreLookup.get(`${dim.name}::${entityB?.name}`) ?? 0;
    ...
    dimensions: dimensions.map((dim) => {
      return {
        subject: dim.name,
        a: scoreLookup.get(`${dim.name}::${entityA?.name}`) ?? 50,
        b: scoreLookup.get(`${dim.name}::${entityB?.name}`) ?? 50,
        fullMark: 100,
      };
    }),
...
async function buildPartialResult(
  ...
  try {
    ...
    for (const s of scores) {
      ...
      if (dim) {
        scoreLookup.set(`${dim.name}::${s.entityId}`, Number(s.score));
      }
    }

    const categories = Array.from(factsByDim.entries()).map(([name, dimFacts]) => {
      const aScore = scoreLookup.get(`${name}::${entityA.id}`) ?? 50;
      const bScore = scoreLookup.get(`${name}::${entityB.id}`) ?? 50;
      return {
        name,
        winner: aScore > bScore ? "a" : bScore > aScore ? "b" : "tie",
