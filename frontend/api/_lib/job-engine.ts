...
function buildResultJson(
  ...
  for (const f of facts) {
    ...
    if (!factsByDim.has(dim)) factsByDim.set(dim, []);
    factsByDim.get(dim)!.push(f);
  }
...
async function buildPartialResult(
  ...
  try {
    ...
    // Precompute dimension lookups to avoid O(N*M) lookups
    const dimNameById = new Map(dims.map(d => [d.id, d.name]));
    const scoresByDimEntity = new Map<string, number>();
    for (const s of scores) {
      const dimName = dimNameById.get(s.dimensionId);
      if (dimName) {
        const val = Number(s.score);
        scoresByDimEntity.set(`${dimName}:${s.entityId}`, isNaN(val) ? 50 : val);
      }
    }
    ...
    const categories = Array.from(factsByDim.entries()).map(([name, dimFacts]) => {
      const aScore = scoresByDimEntity.get(`${name}:${entityA.id}`) ?? 50;
      const bScore = scoresByDimEntity.get(`${name}:${entityB.id}`) ?? 50;
      return {
        name,
        winner: aScore > bScore ? "a" : bScore > aScore ? "b" : "tie",
