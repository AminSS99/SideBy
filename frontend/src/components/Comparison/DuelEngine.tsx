...
const DuelEngine = ({ userCredits, onSpendCredit }: DuelEngineProps) => {
  ...
  const startDuel = async () => {
    ...
    try {
      // NEW: Send both items to the comparison API
      const response = await fetch(
        `http://localhost:8080/api/compare?itemA=${encodeURIComponent(finalA.name)}&itemB=${encodeURIComponent(finalB.name)}`
      );
      const data = await response.json();
      
      // Store the AI-generated comparison summary
      setAiSummary(data.summary);
      toast.success("Comparison Complete", {
        description: `${finalA.name} vs ${finalB.name} analyzed by Gemini AI.`
      });
    } catch (error) {
      console.error("Failed to fetch from backend:", error);
      setAiSummary(null);
      toast.error("Neural Network Offline", {
        description: "Falling back to local synthetic data."
      });
    }
  };
      ...
      {showResult && (
        <div className="space-y-12 mb-20 animate-in fade-in slide-in-from-top-4 duration-700">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-8">
            <AIVerdict itemA={itemA} itemB={itemB} weights={weights} summary={aiSummary} />
          ...
          {/* Neural Calibration removed - AI now auto-analyzes based on item types */}
