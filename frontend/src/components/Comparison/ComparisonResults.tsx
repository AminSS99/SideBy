...
import { 
  Trophy, ArrowLeftRight, Share2, ChevronLeft, ChevronRight, 
  Sparkles, Check, Crown, Zap, Target, Shield, TrendingUp,
  Star, MessageCircle, Copy, ExternalLink, Award, Flame,
  ChevronDown, ChevronUp, Globe, Users, DollarSign, Heart,
  Building, Clock, GraduationCap, User, Wallet, Monitor, Gauge
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { getAnimationConfig, isMobileDevice, prefersReducedMotion } from "@/utils/optimizations";
import { detectCategory, getRandomTip, CategoryConfig } from "@/config/categoryConfig";
...
const getScoreStyle = (score: number) => {
  if (score >= 85) return { color: "text-blue-400", bg: "bg-blue-500", border: "border-blue-500/50", glow: "shadow-blue-500/40", label: "Excellent" };
  if (score >= 70) return { color: "text-emerald-400", bg: "bg-emerald-500", border: "border-emerald-500/50", glow: "shadow-emerald-500/40", label: "Great" };
  if (score >= 55) return { color: "text-yellow-400", bg: "bg-yellow-500", border: "border-yellow-500/50", glow: "shadow-yellow-500/40", label: "Good" };
  if (score >= 40) return { color: "text-orange-400", bg: "bg-orange-500", border: "border-orange-500/50", glow: "shadow-orange-500/40", label: "Fair" };
  return { color: "text-red-400", bg: "bg-red-500", border: "border-red-500/50", glow: "shadow-red-500/40", label: "Poor" };
};

// Animated score with glow
const AnimatedScore = ({ value, delay = 0, size = "lg" }: { value: number; delay?: number; size?: string }) => {
  ...
  return (
    <motion.span 
      ...
      animate={{ textShadow: [`0 0 20px ${style.color}`, `0 0 40px ${style.color}`, `0 0 20px ${style.color}`] }}
...
const parseAISummary = (summary: string | null, itemA: string, itemB: string) => {
  const defaultResult = {
    category: "General",
    verdict: `Both ${itemA} and ${itemB} are excellent choices.`,
    winner: itemA,
    itemAStrengths: ["Strong foundation", "Proven track record", "Wide adoption"],
  ...
  try {
    ...
    const category = categoryLine?.replace(/.*category:\s*/i, '').replace(/[#*]/g, '').trim() || "General";
    
    // Extract verdict and winner
    const verdictLine = lines.find(l => l.includes('Verdict') || l.includes('verdict'));
    const verdictIdx = lines.findIndex(l => l.includes('Verdict') || l.includes('verdict'));
    let verdict = "";
    if (verdictIdx !== -1 && lines[verdictIdx + 1]) {
      verdict = lines[verdictIdx + 1].replace(/[#*]/g, '').trim();
    }
    if (!verdict && verdictLine) {
      verdict = verdictLine.replace(/.*Verdict/i, '').replace(/[#*🏆]/g, '').trim();
    }
    
    // Determine winner from verdict
    let winner = itemA;
    if (verdict.toLowerCase().includes(itemB.toLowerCase()) && 
        (verdict.toLowerCase().includes('better') || verdict.toLowerCase().includes('wins') || verdict.toLowerCase().includes('winner'))) {
      winner = itemB;
    } else if (verdict.toLowerCase().includes(itemA.toLowerCase()) && 
               (verdict.toLowerCase().includes('better') || verdict.toLowerCase().includes('wins') || verdict.toLowerCase().includes('winner'))) {
      winner = itemA;
    }
    ...
    let itemAIdx = lines.findIndex(l => itemAPatterns.some(p => l.toLowerCase().includes(p.toLowerCase())));
    const itemAStrengths: string[] = [];
    
    if (itemAIdx !== -1) {
      for (let i = itemAIdx + 1; i < Math.min(itemAIdx + 12, lines.length); i++) {
        const line = lines[i].trim();
        // Stop if we hit another section
        if (line.toLowerCase().includes(itemB.toLowerCase()) && (line.includes('##') || line.includes('**'))) break;
        if (line.toLowerCase().includes('recommendation') || line.toLowerCase().includes('verdict')) break;
        ...
        if (line.startsWith('-') || line.startsWith('•') || line.startsWith('*') || 
          ...
          const strength = line
            ...
            .replace(/\*\*/g, '')
            .trim();
          if (strength && strength.length > 5 && !strength.toLowerCase().includes('best for')) {
    ...
    let itemBIdx = lines.findIndex(l => itemBPatterns.some(p => l.toLowerCase().includes(p.toLowerCase())));
    const itemBStrengths: string[] = [];
    if (itemBIdx !== -1) {
      for (let i = itemBIdx + 1; i < Math.min(itemBIdx + 12, lines.length); i++) {
        const line = lines[i].trim();
    ...
    const recIdx = lines.findIndex(l => l.includes('Recommendation') || l.includes('Final'));
    let recommendation = "";
    if (recIdx !== -1) {
    ...
    return {
      ...
      winner,
      itemAStrengths: itemAStrengths.length > 0 ? itemAStrengths : defaultResult.itemAStrengths,
      itemBStrengths: itemBStrengths.length > 0 ? itemBStrengths : defaultResult.itemBStrengths,
...
const ComparisonResults: React.FC<ComparisonResultsProps> = ({
  ...
  const parsedAI = useMemo(() => parseAISummary(aiSummary, itemA, itemB), [aiSummary, itemA, itemB]);
  ...
  const detectedCategory = useMemo(() => detectCategory(itemA, itemB), [itemA, itemB]);
  
  // Dynamic tip based on category
  const categoryTip = useMemo(() => getRandomTip(detectedCategory), [detectedCategory]);
  
  // Icon mapping for dynamic categories
  const iconMap: Record<string, any> = {
    Trophy, Award, DollarSign, Users, TrendingUp, Globe, Shield, Star,
    Flame, Heart, Zap, Building, Clock, GraduationCap, User, Wallet, Monitor, Gauge, Target
  };
  ...
  const categories = useMemo(() => {
    ...
    return metrics.map((metric, i) => ({
      name: metric.label.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
  ...
  const handleShare = () => {
    navigator.clipboard.writeText(`https://sideby.app/compare/${itemA}-vs-${itemB}`);
    setCopied(true);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 relative">
      ...
      <GlowOrb color="bg-purple-600" size="w-96 h-96" position="-top-48 -left-48" />
      <GlowOrb color="bg-emerald-600" size="w-80 h-80" position="-bottom-40 -right-40" />
            ...
            <div className="relative px-8 py-4 rounded-2xl bg-gradient-to-r from-yellow-500/20 via-orange-500/20 to-yellow-500/20 border border-yellow-500/30 backdrop-blur-sm">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="text-xs text-yellow-300/70 font-bold uppercase tracking-wider">Champion</div>
                  <div className="text-2xl font-black text-white">{winner}</div>
                </div>
                <div className="ml-4 text-right">
                  <div className="text-3xl font-black text-emerald-400">92%</div>
                  <div className="text-[10px] text-white/50 uppercase">Match Score</div>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        >
          <motion.div 
            ...
            animate={winnerIsA ? { scale: [1, 1.05, 1], opacity: [0.3, 0.5, 0.3] } : {}}
            transition={{ duration: 2, repeat: Infinity }}
          />
          <GlassCard className={`relative p-6 rounded-3xl border-2 transition-all ${winnerIsA ? 'border-purple-400/60' : 'border-white/10'}`}>
            {winnerIsA && (
              <motion.div 
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
            ...
            <div className="text-center">
              ...
              <h3 className="text-2xl font-black tracking-tight mb-1">{itemA}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                {categories.slice(0, 2).map((cat, i) => {
                  ...
                  return (
                    ...
                    >
                      <cat.icon className={`w-4 h-4 mx-auto mb-1 ${style.color}`} />
                      <AnimatedScore value={cat.scoreA} delay={600 + i * 100} size="lg" />
                      <div className="text-[9px] text-white/40 uppercase mt-1">{cat.name}</div>
                    </motion.div>
                  );
                })}
              </div>
        ...
        >
          <motion.div 
            ...
            transition={{ duration: 2, repeat: Infinity }}
          />
          <GlassCard className={`relative p-6 rounded-3xl border-2 transition-all ${!winnerIsA ? 'border-emerald-400/60' : 'border-white/10'}`}>
            {!winnerIsA && (
              <motion.div 
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
            ...
            <div className="text-center">
              ...
              <h3 className="text-2xl font-black tracking-tight mb-1">{itemB}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                {categories.slice(0, 2).map((cat, i) => {
                  ...
                  return (
                    ...
                    >
                      <cat.icon className={`w-4 h-4 mx-auto mb-1 ${style.color}`} />
                      <AnimatedScore value={cat.scoreB} delay={600 + i * 100} size="lg" />
                      <div className="text-[9px] text-white/40 uppercase mt-1">{cat.name}</div>
                    </motion.div>
                  );
                })}
            ...
            {expandedSection === "breakdown" && (
              ...
              >
                ...
                <div className="flex justify-center gap-4 pt-4 text-[10px]">
                  {["Excellent", "Great", "Good", "Fair", "Poor"].map((label, i) => {
                    const colors = ["bg-blue-500", "bg-emerald-500", "bg-yellow-500", "bg-orange-500", "bg-red-500"];
                    return (
        ...
        <motion.div whileHover={{ scale: 1.02, y: -5 }}>
          <GlassCard className="p-5 rounded-2xl h-full bg-gradient-to-br from-purple-600/10 to-transparent border-purple-500/20 hover:border-purple-400/40 transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <span className="text-xs font-black uppercase text-purple-300">Best For</span>
              </div>
            </div>
            <div className="space-y-2">
              <p className="text-sm"><span className="text-purple-400 font-bold">{itemA}:</span> <span className="text-white/70">{detectedCategory.bestForLabels.itemA}</span></p>
              <p className="text-sm"><span className="text-emerald-400 font-bold">{itemB}:</span> <span className="text-white/70">{detectedCategory.bestForLabels.itemB}</span></p>
            </div>
          </GlassCard>
        </motion.div>
        ...
        <motion.div whileHover={{ scale: 1.02, y: -5 }}>
          <GlassCard className="p-5 rounded-2xl h-full bg-gradient-to-br from-emerald-600/10 to-transparent border-emerald-500/20 hover:border-emerald-400/40 transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <span className="text-xs font-black uppercase text-emerald-300">Trending</span>
              </div>
            </div>
            <p className="text-sm text-white/80">
              <span className="text-emerald-400 font-bold">{winner}</span> leads overall by <motion.span 
                className="text-2xl font-black text-emerald-400"
                animate={{ scale: [1, 1.1, 1] }}
                transition={{ duration: 1, repeat: Infinity }}
              >+6</motion.span> points
            </p>
          </GlassCard>
        </motion.div>
        ...
        <motion.div whileHover={{ scale: 1.02, y: -5 }}>
          <GlassCard className="p-5 rounded-2xl h-full bg-gradient-to-br from-yellow-600/10 to-transparent border-yellow-500/20 hover:border-yellow-400/40 transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <span className="text-xs font-black uppercase text-yellow-300">Pro Tip</span>
              </div>
            </div>
        ...
        <GlassCard className="p-6 rounded-3xl overflow-hidden">
          ...
          >
            <h4 className="text-sm font-black uppercase tracking-widest text-white/60 flex items-center gap-2">
              ...
              AI Deep Analysis
              <span className="ml-2 px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 text-[10px]">Detailed</span>
            </h4>
            {expandedSection === "analysis" ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
          </button>
              ...
              >
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="text-xs font-black uppercase text-purple-300 mb-1">🏆 Quick Verdict</div>
                      <p className="text-sm text-white/90 leading-relaxed">
                        {parsedAI.verdict || (
                          <><span className="text-white font-bold">{winner}</span> is the better choice for most users.</>
                        )}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-bold text-purple-300">{itemA} Strengths</span>
                    </div>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-bold text-emerald-300">{itemB} Strengths</span>
                    </div>
                    <ul className="space-y-2">
                ...
                >
                  {[
                    { icon: Users, label: "Community", valueA: "Massive", valueB: "Growing" },
                    { icon: TrendingUp, label: "Momentum", valueA: "Stable", valueB: "Rising" },
                    { icon: Award, label: "Learning", valueA: "Steeper", valueB: "Easier" },
                    { icon: Shield, label: "Stability", valueA: "Battle-tested", valueB: "Mature" },
                  ].map((insight, i) => (
                    <motion.div
                      key={insight.label}
                    ...
                    >
                      ...
                      <div className="text-[10px] font-bold text-white/40 uppercase mb-1">{insight.label}</div>
                      <div className="text-[10px]">
                        <span className="text-purple-400">{insight.valueA}</span>
                        <span className="text-white/30 mx-1">vs</span>
                        <span className="text-emerald-400">{insight.valueB}</span>
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs font-black uppercase text-yellow-300 mb-1">💡 Final Recommendation</div>
                      <p className="text-sm text-white/80 leading-relaxed">
                        {parsedAI.recommendation || (
                          <>Choose <span className="text-purple-400 font-bold">{itemA}</span> for larger projects. 
                          Choose <span className="text-emerald-400 font-bold">{itemB}</span> for rapid development.</>
                        )}
                      </p>
                    </div>
                  </div>
