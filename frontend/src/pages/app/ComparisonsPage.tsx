...
const ComparisonsPage = () => {
  ...
  const startComparison = async (value = newComparison) => {
    ...
    try {
      ...
      toast.error("Research could not start.", {
        description:
          creationError instanceof Error
            ? creationError.message
            : "Unable to start comparison research.",
      });
    } finally {
      setIsCreating(false);
    }
        ...
        <div className="rounded-[24px] border border-white/10 bg-black/30 p-2 sm:rounded-[28px]">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <button
              ...
              disabled={isCreating}
              className="inline-flex h-10 items-center justify-center rounded-2xl bg-white px-4 text-xs font-bold uppercase tracking-[0.2em] text-black transition-colors hover:bg-white/90 disabled:opacity-50"
            >
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2 lg:justify-end">
          <Link
            ...
            className="inline-flex h-10 items-center gap-2 rounded-2xl bg-white px-4 text-xs font-bold uppercase tracking-[0.2em] text-black transition-colors hover:bg-white/90"
          >
            Review
          </Link>
          {item.visibility === "public" ? (
            ...
            <button
              ...
              className="inline-flex h-10 items-center gap-2 rounded-2xl border border-white/10 px-4 text-xs font-bold uppercase tracking-[0.2em] text-white/60 transition-colors hover:border-emerald-300/40 hover:text-emerald-200 disabled:cursor-not-allowed disabled:opacity-40"
            >
              {isPublishing ? (
